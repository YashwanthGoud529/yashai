...
export default function AuthModal({ isOpen, onClose, onAuthSuccess }) {
  ...
  // Execute Google Authentication directly into MongoDB Atlas
  const authenticateGoogleAccount = async (accountName, accountEmail, avatar) => {
    setError(null);
    setSuccessMsg(null);
    ...
    try {
      ...
      const cleanName = (accountName || cleanEmail.split("@")[0]).trim();
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/80 backdrop-blur-xs animate-in fade-in duration-150">
      ...
      >
        {/* Google Account Picker Sub-Modal */}
        <AnimatePresence>
          {googleChooserOpen && (
            <motion.div
              ...
              exit={{ opacity: 0, y: 15 }}
              className="absolute inset-0 bg-[#0d1117] z-20 flex flex-col p-6 overflow-y-auto"
            >
              <div className="flex items-center justify-between pb-3 border-b border-slate-800">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h4 className="font-semibold text-xs text-white">Choose an account</h4>
                    <p className="text-[10px] text-slate-400">to continue to Yash AI</p>
                  </div>
                </div>
                <button
                  type="button"
                  onClick={() => setGoogleChooserOpen(false)}
                  className="p-1 rounded-[4px] text-slate-400 hover:text-white hover:bg-slate-800"
                >
                  <X className="w-4 h-4" />
                </button>
              </div>
              ...
              <div className="py-4 space-y-2 flex-1">
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.98 }}
                  type="button"
                  onClick={() => authenticateGoogleAccount("Yashwanth Goud", "yashwanthj111@gmail.com", "https://lh3.googleusercontent.com/a/ACg8ocIq7H3example")}
                  disabled={googleLoading}
                  className="w-full flex items-center justify-between p-3 rounded-[4px] bg-slate-900 border border-slate-800 hover:border-[#3e55af] transition-all text-left group"
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-xs font-semibold text-white">Yashwanth Goud</div>
                      <div className="text-[11px] text-slate-400">yashwanthj111@gmail.com</div>
                    </div>
                  </div>
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.98 }}
                  type="button"
                  onClick={() => authenticateGoogleAccount("Yashwanth Goud", "yashwanthgoud529@gmail.com", "https://lh3.googleusercontent.com/a/ACg8ocIq7H3example")}
                  disabled={googleLoading}
                  className="w-full flex items-center justify-between p-3 rounded-[4px] bg-slate-900 border border-slate-800 hover:border-[#3e55af] transition-all text-left group"
                >
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-blue-600 to-indigo-600 flex items-center justify-center text-white text-xs font-semibold">
                      YG
                    </div>
                    <div>
                      <div className="text-xs font-semibold text-white">Yashwanth Goud</div>
                      <div className="text-[11px] text-slate-400">yashwanthgoud529@gmail.com</div>
                    </div>
                  </div>
                  <ChevronRight className="w-4 h-4 text-slate-500 group-hover:text-blue-400 transition-colors" />
                </motion.button>
                ...
                <div className="pt-2 border-t border-slate-800/80 space-y-2">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-950 border border-slate-800 focus:border-[#3e55af] rounded-[4px] px-3 py-1.5 text-xs text-slate-200 placeholder:text-slate-600 outline-none"
                  />
                  ...
                  {customGoogleEmail && (
                    <motion.button
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      type="button"
                      onClick={() => authenticateGoogleAccount(customGoogleName || customGoogleEmail.split("@")[0], customGoogleEmail)}
                      disabled={googleLoading}
                      className="w-full py-1.5 bg-brand hover-bg-brand text-white text-xs font-semibold rounded-[4px] transition-all"
                    >
                      Continue as {customGoogleEmail}
                    </motion.button>
                  )}
                </div>
              </div>
              ...
              <div className="pt-3 border-t border-slate-800 text-center text-[10px] text-slate-500">
                To continue, Google will share your name, email address, and profile picture with Yash AI.
              </div>
            </motion.div>
          )}
        </AnimatePresence>
