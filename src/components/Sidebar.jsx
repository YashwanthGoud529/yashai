      ...
      >
        ...
        <div className="p-3.5 space-y-2.5 border-b border-slate-800">
          ...
          <motion.button
            ...
            className="w-full flex items-center justify-between px-3 py-2 rounded-[4px] bg-brand hover-bg-brand text-white text-xs font-semibold shadow-sm transition-all group"
          >
          ...
          {conversations.length > 0 && (
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-slate-900 border border-slate-800 rounded-[4px] pl-7 pr-2.5 py-1.5 text-xs text-slate-200 placeholder:text-slate-500 outline-none focus:border-[#3e55af] font-normal"
              />
            </div>
          )}
        </div>
                    ...
                    return (
                      <motion.div
                        ...
                        className={`group relative flex items-center justify-between px-2.5 py-2 rounded-[4px] cursor-pointer text-xs transition-all ${
                          isSelected
                            ? "bg-slate-800 text-white font-semibold shadow-xs border-l-2 border-l-[#024dbe] border-y border-r border-slate-700/60"
                            : "text-slate-400 hover:bg-slate-900 hover:text-slate-200 font-normal border-l-2 border-l-transparent"
                        }`}
                      >
                        <div className="flex items-center gap-2 min-w-0 flex-1">
                          <MessageSquare className={`w-3.5 h-3.5 shrink-0 ${isSelected ? "text-blue-400" : "text-slate-500"}`} />
                          ...
                          {isEditing ? (
                            <input
                              ...
                              autoFocus
                              className="bg-slate-950 text-white px-1.5 py-0.5 rounded-[4px] border border-[#3e55af] outline-none w-full text-xs"
                            />
                          ) : (
        ...
        <div className="p-2.5 border-t border-slate-800 bg-[#0d1117] space-y-1.5">
          ...
          {/* Professional User Profile Card (Direct MongoDB Atlas Cloud Sync) */}
          {user ? (
            <div className="p-2.5 rounded-[4px] bg-slate-950 border border-slate-800/90 shadow-md space-y-2">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2.5 min-w-0 flex-1">
                  <div className="w-8 h-8 rounded-[4px] bg-brand flex items-center justify-center text-white text-xs font-semibold shadow-xs shrink-0 overflow-hidden border border-[#3e55af]">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="text-xs font-semibold text-white truncate leading-tight flex items-center gap-1.5">
                      <span>{user.name || "User"}</span>
                      <ShieldCheck className="w-3 h-3 text-emerald-400 shrink-0" />
                    </div>
                    <div className="text-[10px] text-slate-400 truncate leading-tight font-normal mt-0.5">
                      {user.email}
                    </div>
                  </div>
                </div>
                ...
                <button
                  onClick={onLogout}
                  className="p-1.5 rounded-[4px] text-slate-400 hover:text-rose-400 hover:bg-rose-950/30 transition-colors shrink-0 ml-1"
                  title="Log out"
                >
              ...
              <div className="flex items-center justify-between pt-1.5 border-t border-slate-800/60 text-[10px]">
                ...
                <span className="text-slate-500 uppercase font-mono text-[9px] px-1 py-0.2 rounded-[2px] bg-slate-900 border border-slate-800 font-semibold">
                  {user.provider || "Local"}
                </span>
              </div>
            </div>
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              onClick={onOpenAuth}
              className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-[4px] bg-[#3e55af]/20 hover:bg-[#3e55af]/30 border border-[#3e55af]/50 text-blue-200 hover:text-white text-xs font-semibold transition-all shadow-xs"
            >
